...
class Gmap {
	...
	/**
     * CARLOS OLMOS: usa la posición actual del GPS para el centro.
     *
     * @var bool
     */
    var $centerOnCurrentPos=true;
	...
	*/
	 ...
	 * Carlos Olmos: This function has been modified to add draggable marker and a callback function.
     */
    function getCreateMarkerJS() {
    	$_output = "
    	   function createMarker(map, point, title, html, icon, icon_shadow, sidebar_id, openers){
				...
				google.maps.event.addListener(new_marker, 'dragend', function(evt){
					endDragHandler(evt.latLng.lat().toFixed(6) , evt.latLng.lng().toFixed(6), this);
				});
				
				google.maps.event.addListener(new_marker, 'dblclick', function(evt) {
					dblClickHandler(evt.latLng.lat().toFixed(6) , evt.latLng.lng().toFixed(6), this);
				});
	...
	/**
	* Carlos Olmos: Función para detectar la posición del usuario.
	* 
	*/
    ...
    function getMap() {
        ...
        if(strlen($this->width) > 0 && strlen($this->height) > 0) {
            $_output .= sprintf('document.write(\'<div id="%s" class="mapcanvas" ><\/div>\');',$this->map_id,$this->width,$this->height) . "\n";
        } else {
            $_output .= sprintf('document.write(\'<div id="%s" class="mapcanvas" ><\/div>\');',$this->map_id) . "\n";     
        }
        //$_output .= '}';
